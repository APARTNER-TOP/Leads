    ...
    <form method="post" action="{{ route('api.send2') }}" class="mt-6 space-y-6">
        ...
        <div class="mt-2 mb-2">
            ...
            <select id="type_user" name="lead_source" class="form-control rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full">
                ...
                <option value="{{ $source->id }}" @if(old('lead_source') ==  $source->id ) selected @endif>
                    {{ $source->name }}
                    <!-- {{ $source->email }} -->
                </option>
                @endforeach
        ...
        <div class="mt-2 mb-2">
            ...
            <x-text-input id="origin_city" name="origin_city" type="text" class="mt-1 block w-full" :value="old('origin_city')" required autofocus autocomplete="origin_city" />
        ...
        <div class="mt-2 mb-2">
            ...
            <x-text-input id="origin_state" name="origin_state" type="text" class="mt-1 block w-full" :value="old('origin_state')" required autofocus autocomplete="origin_state" />
            <x-input-error class="mt-2" :messages="$errors->get('origin_state')" />
        </div>

        <div class="mt-2 mb-2">
            <x-input-label for="origin_zip" :value="__('Origin zip')" />
            <x-text-input id="origin_zip" name="origin_zip" type="number" class="mt-1 block w-full" :value="old('origin_zip')" required autofocus autocomplete="origin_zip" />
            <x-input-error class="mt-2" :messages="$errors->get('origin_zip')" />
        </div>

        <div class="mt-2 mb-2">
            <x-input-label for="destination_city" :value="__('Destination city')" />
            <x-text-input id="destination_city" name="destination_city" type="text" class="mt-1 block w-full" :value="old('destination_city')" required autofocus autocomplete="destination_city" />
        ...
        <div class="mt-2 mb-2">
            ...
            <x-text-input id="destination_state" name="destination_state" type="text" class="mt-1 block w-full" :value="old('destination_state')" required autofocus autocomplete="destination_state" />
            <x-input-error class="mt-2" :messages="$errors->get('destination_state')" />
        </div>

        <div class="mt-2 mb-2">
            <x-input-label for="destination_zip" :value="__('Destination zip')" />
            <x-text-input id="destination_zip" name="destination_zip" type="number" class="mt-1 block w-full" :value="old('destination_zip')" required autofocus autocomplete="destination_zip" />
